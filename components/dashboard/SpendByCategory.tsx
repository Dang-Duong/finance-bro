...
export default function SpendByCategory() {
  const highlightedCategory = mockData[0]; // Food is highlighted
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold mb-4 text-white">Spend by Category</h2>
      ...
      <div className="relative h-64">
            ...
            >
              {mockData.map((entry, index) => (
                <Cell
                  ...
                  fill={entry.color}
                  style={{
                    filter: entry.name === highlightedCategory.name
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          ...
          >
            ...
            <div className="text-white/80 text-sm">{highlightedCategory.name}</div>
          </motion.div>
        </div>
      </div>
    </motion.div>
