...
interface AddGoalModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { name: string; goalAmount: number }) => Promise<void>;
...
export default function AddGoalModal({
  ...
  useEffect(() => {
    if (editingGoal && isOpen) {
      ...
      setGoalAmount(String(editingGoal.goalAmount));
    }
  }, [editingGoal, isOpen]);

  useEffect(() => {
    if (!isOpen || (isOpen && !editingGoal)) {
      setName("");
      setGoalAmount("");
      setError("");
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to submit goal";
